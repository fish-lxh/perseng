/**
 * @promptx/logger - Unified logging system for Perseng using Pino
 * Features:
...
const defaultConfig: LoggerConfig = {
  ...
  console: true,
  file: {
    dirname: path.join(os.homedir(), '.perseng', 'logs')
  },
  colors: true
}
...
export function createLogger(config: LoggerConfig = {}): pino.Logger {
  ...
  if (finalConfig.file) {
    ...
    const logDir = fileConfig.dirname || path.join(os.homedir(), '.perseng', 'logs')
  ...
  if (forceSyncLogging) {
    ...
    if (finalConfig.file) {
      ...
      const logPath = path.join(logDir, `perseng-${today}.log`)
    ...
    if (finalConfig.file) {
      ...
      targets.push({
        ...
        level: finalConfig.level,
        options: {
          destination: path.join(logDir, `perseng-${today}.log`)
        }
      })
      ...
      targets.push({
        ...
        level: 'error',
        options: {
          destination: path.join(logDir, `perseng-error-${today}.log`)
        }
      })
    }
