 ...
 * 
 * 1. 架构隔离性
 * 专为 Perseng 体系设计，通过沙箱隔离确保文件操作不会影响
 * Perseng 核心功能。即使 AI Agent 出错，也不会破坏系统稳定性。
 *
 * 2. 平台独立性
 * 虽然很多 AI 平台自带文件工具，但 Perseng 需要自己的实现来保证：
 * - 在无本地工具的 Web Agent 平台上也能工作
 * - 统一的操作语义，不依赖特定 AI 平台
 * - 可移植到任何支持 MCP 协议的环境
 *
 * 3. 生态自主性
 * 作为 Perseng 工具生态的基础组件，filesystem 确保了：
 * - 其他工具可以依赖稳定的文件操作接口
 * - 用户数据始终在 Perseng 控制范围内
 * - 未来可扩展更多存储后端（云存储、分布式等）
 *
 * 这不仅是一个文件操作工具，更是 Perseng 实现平台独立、
 * 生态自主的关键基础设施。
 */
...
module.exports = {
    ...
    return {
      ...
      environment: {
        ...
        properties: {
          ALLOWED_DIRECTORIES: {
            type: 'string',
            description: '允许访问的目录列表（JSON数组格式），默认为 ["~/.perseng"]',
            default: '["~/.perseng"]'
          }
        },
        required: []
  ...
  getAllowedDirectories() {
    ...
    let allowedDirs = ['~/.perseng'];  // Default value
  ...
  async execute(params) {
    ...
    try {
      ...
      // 路径参数转换
      if (params.path) {
        mcpParams.path = this.resolvePersengPath(params.path);
      }
      
      if (params.paths) {
        mcpParams.paths = params.paths.map(p => this.resolvePersengPath(p));
      }
      
      if (params.source) {
        mcpParams.source = this.resolvePersengPath(params.source);
      }
      
      if (params.destination) {
        mcpParams.destination = this.resolvePersengPath(params.destination);
      }
