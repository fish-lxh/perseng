...
DPML#资源提示单元 框架用于定义特定类型的#资源单元，使开发者能够以标准化的方式在提示词中描述如何#定位 和处理各种资源。通过这个框架，可以明确#资源协议 的引用语法、#位置 规则和#查询参数，确保#资源引用 在不同环境中的一致性和可靠性。此框架是Perseng中#资源协议（RP）在提示词层面的具体实现方式。
...
Perseng默认支持以下通用且已有共识的#资源协议，这些协议符合`@协议名://路径`格式，遵循其业界标准语法和规则，无需在#资源单元 中重新定义：
...
<resource protocol="thought">
  ...
  <registry>
    | 思维ID | 文件路径 |
    |--------|---------|
    | analytical | @file://Perseng/core/thoughts/analytical.thought.md |
    | creative | @file://Perseng/core/thoughts/creative.thought.md |
  </registry>
</resource>
```
