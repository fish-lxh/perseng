 ...
 * 迁出。querySqlite 之前在内部调 getPersengHomeDir(), 现在显式接收 homeDir
 * 参数, 让本包不依赖 ~/main/utils/persengPaths (即不依赖 electron 宿主层)。
 */
...
export interface DbItem {
  ...
  relativePath: string      // 相对 rootDir 的路径
  type: 'sqlite' | 'json'
  size: number              // bytes
...
/**
 * 递归扫描给定 rootDir，返回所有 .db / .json 文件的元信息列表。
 *
 * 调用方传入 rootDir (通常 = ~/.perseng/), 内部不再依赖任何宿主路径 API。
 */
 ...
 *
 * 4 道安全防线：
 *  1. 路径白名单：dbPath 必须在 homeDir 下
 *  2. readonly 连接：写操作直接被 sqlite 拒绝
 *  3. 单语句：prepare() 不支持多语句，多 `;` 抛错
 *  4. LIMIT 守卫：用户没写 LIMIT 时自动追加 MAX_ROWS+1 行上限
 *
 * KNUTH-FEAT 2026-07-10: homeDir 改成显式参数, 不再内部调用
 * getPersengHomeDir(), 让本函数可在非 electron 宿主环境复用。
 */
export function querySqlite(dbPath: string, sql: string, homeDir: string): SqlQueryResult {
  // 1. 路径白名单
  if (!dbPath.startsWith(homeDir)) {
    throw new Error('Path not allowed: dbPath must be under the configured homeDir')
  }
