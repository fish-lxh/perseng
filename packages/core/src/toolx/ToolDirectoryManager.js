...
class ToolDirectoryManager {
  constructor(toolId, resourceManager) {
    ...
    // 使用协议定义目录
    this.directories = {
      working: '@user://.perseng',                           // 工作目录
      toolbox: `@user://.perseng/toolbox/${toolId}`,        // 工具隔离目录
      dependencies: `@user://.perseng/toolbox/${toolId}/node_modules`, // 依赖目录
      cache: `@user://.perseng/cache/${toolId}`,            // 缓存目录
      temp: `@user://.perseng/temp/${toolId}`,              // 临时文件目录
    };
  ...
  /**
   * 通过协议解析路径
   * @param {string} protocolPath - 协议路径，如 @user://.perseng
