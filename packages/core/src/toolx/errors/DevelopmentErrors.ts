...
interface ErrorDefinition {
  ...
  category: string
  description: string
  identify: (error: Error, context?: ToolErrorContext) => boolean
  getSolution: (error: Error, context?: ToolErrorContext) => SolutionDetail
}

// eslint-disable-next-line @typescript-eslint/no-namespace
namespace DevelopmentErrorsNs {
  export const DEVELOPMENT_ERRORS: Record<string, ErrorDefinition> = {
      ...
      identify: (error, context = {}) => {
        ...
        if (context.dependencies) {
          if (typeof context.dependencies === 'object') {
            return !Object.keys(context.dependencies as Record<string, unknown>).includes(moduleName)
          }
        }
  ...
  return {
    ...
    TOOL_SYNTAX_ERROR: {
      ...
      getSolution: (error) => {
        ...
        const line = lineMatch?.[1] ?? 'unknown'
...
export = DevelopmentErrorsNs.DEVELOPMENT_ERRORS