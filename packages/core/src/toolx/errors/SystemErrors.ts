/**
 * SystemErrors - 系统和环境相关错误定义
 *
 * 这些错误通常是 Perseng 系统问题或环境问题。
 *
...
interface SystemErrorDefinition {
  code: string
  category: string
  description: string
  identify: (error: Error & { code?: string }, context?: SystemErrorContext) => boolean
  getSolution: (error: Error, context?: SystemErrorContext) => SystemSolution
}

// eslint-disable-next-line @typescript-eslint/no-namespace
namespace SystemErrorsNs {
  export const SYSTEM_ERRORS: Record<string, SystemErrorDefinition> = {
    ...
    NETWORK_TIMEOUT: {
      ...
      getSolution: (_error, context = {}) => {
        ...
        const newTimeout = Math.min(currentTimeout * 2, 300000)
    ...
    TOOL_NOT_FOUND: {
      ...
      getSolution: (_error, context = {}) => {
        return {
          message: '指定的工具不存在',
          toolId: context.toolId as string | undefined,
          suggestions: [
            '1. 检查工具ID是否正确',
            '2. 确认工具已安装',
    ...
    UNKNOWN_ERROR: {
      ...
      getSolution: (error, context = {}) => {
        return {
          ...
          stack: error.stack,
          context: context as SystemErrorContext,
          suggestions: [
            '1. 查看错误详情',
            '2. 重试操作',
            '3. 报告问题给开发者',
...
export = SystemErrorsNs.SYSTEM_ERRORS