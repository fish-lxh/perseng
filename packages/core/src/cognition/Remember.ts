 ...
 * @class Remember
 */
import { debug, info, warn } from '@promptx/logger'
import { Engram } from './Engram'
import { WeightContext } from './WeightContext'
import { WeightStrategy } from './WeightStrategy'
import { Network } from './Network'

export interface RememberConnection {
  source: string
  target: string
  weight: number
  position: number
...
export interface RememberResult {
  processed: number
  connections: RememberConnection[]
  timestamp?: number
  engramId?: string
}

export interface RememberOptions {
  strategy?: WeightStrategy | null
}
...
export class Remember {
  ...
  /** 权重策略 */
  public strategy: WeightStrategy | null

  constructor(network: Network, options: RememberOptions = {}) {
    this.network = network
    this.strategy = options.strategy ?? null

    if (this.strategy) {
      debug('[Remember] Initialized with strategy', {
        strategy: this.strategy.constructor.name,
      })
    } else {
      warn('[Remember] No strategy provided')
    }
  }
   ...
   */
  execute(engram: Engram, engramId: string | null = null): RememberResult {
    // 参数验证
    if (!engram || !(engram instanceof Engram)) {
      warn('[Remember] Invalid engram provided', { engram })
      return {
        processed: 0,
        connections: [],
      }
    ...
    if (!engram.isValid()) {
      debug('[Remember] Engram schema too short, no connections to create', {
    ...
    debug('[Remember] Processing engram', {
    ...
    // Phase 1: 确保所有Cue存在
    debug('[Remember] Phase 1: Ensuring all Cues exist')
    const createdCues: string[] = []
    ...
    if (createdCues.length > 0) {
      debug('[Remember] Created new Cues', {
    ...
    debug('[Remember] Phase 2: Building connection structure')
    const connections: RememberConnection[] = []
    // 使用engram的timestamp，保证时间一致性

    // 2.1 先建立所有连接（用临时权重0）
    for (let i = 0; i < schema.length - 1; i++) {
      const sourceWord = schema[i] as string
      const targetWord = schema[i + 1] as string
    ...
    debug('[Remember] Phase 2.2: Calculating and updating weights')
    for (let i = 0; i < schema.length - 1; i++) {
      const sourceWord = schema[i] as string
      const targetWord = schema[i + 1] as string
      const sourceCue = this.network.cues.get(sourceWord)

      if (!sourceCue || !this.strategy) continue
      ...
      debug('[Remember] Weight calculation', {
        from: sourceWord,
        to: targetWord,
        position: i,
    ...
    info('[Remember] Schema processed successfully', {
      nodes: schema.length,
    ...
    if (actualEngramId) {
      for (const word of schema) {
        ...
        if (cue) {
          ...
          debug('[Remember] Added engram reference', {
            cue: word,
