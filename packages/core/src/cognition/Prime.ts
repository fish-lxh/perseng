...
import { Recall, RecallOptions } from './Recall'

export interface PrimeWordsOptions {
  maxRootNodes?: number
  childrenPerRoot?: number
  maxTotalWords?: number
}

export class Prime extends Recall {
   ...
   * 3. 返回第一个节点 - 兜底策略
   */
  getPrimeWord(): string | null {
    if (this.network.cues.size === 0) {
      logWarn('[Prime] Network is empty, no word to prime')
      return null
    }

    logDebug('[Prime] Calculating prime word from network', {
    ...
    if (rootNodes.length > 0) {
      // 如果有多个根节点，选择出度最大的那个
      let selectedRoot = rootNodes[0] as string
      let bestOutDegree = this.network.cues.get(selectedRoot)?.connections?.size ?? 0
      for (let i = 1; i < rootNodes.length; i++) {
        const current = rootNodes[i] as string
        ...
        if (currentOutDegree > bestOutDegree) {
          ...
          bestOutDegree = currentOutDegree
        }
      }

      logInfo('[Prime] Selected root node as prime word', {
        word: selectedRoot,
        allRoots: rootNodes,
    ...
    if (inWeights.size > 0) {
      let maxWeight = 0
      let primeWord: string | null = null
      ...
      if (primeWord) {
        logInfo('[Prime] Selected high in-degree node as prime word', {
          word: primeWord,
          inWeight: maxWeight,
        })
        return primeWord
    ...
    // 策略3: 返回第一个节点
    const firstWord = this.network.cues.keys().next().value as string | undefined
    logDebug('[Prime] Using first cue as fallback', {
      word: firstWord,
    })
   ...
   */
  getPrimeWords(options: PrimeWordsOptions = {}): string[] {
    const {
      maxRootNodes = 15,
      childrenPerRoot = 3,
      maxTotalWords = 50,
    ...
    logDebug('[Prime] Getting multiple prime words for DMN mode', {
      maxRootNodes,
      childrenPerRoot,
      maxTotalWords,
    ...
    if (rootNodes.length === 0) {
      logInfo('[Prime] No root nodes found, using hub nodes strategy')
    ...
    logInfo('[Prime] Selected root nodes', {
    ...
    for (const root of sortedRoots) {
      ...
      if (cue && cue.connections.size > 0) {
        ...
        logDebug('[Prime] Added children for root', {
          root,
    ...
    logInfo('[Prime] DMN prime words prepared', {
  ...
  findRootNodes(): string[] {
    ...
    // 标记所有有入边的节点
    for (const [, sourceCue] of this.network.cues) {
    ...
    logDebug('[Prime] Found root nodes', {
   ...
   */
  getTopHubNodes(limit: number): string[] {
    const inWeights = this.network.calculateInWeights()

    if (inWeights.size === 0) {
      logWarn('[Prime] No nodes with in-weights, returning first N nodes')
    ...
    logInfo('[Prime] Selected hub nodes', {
   ...
   *
   * @param word Optional prime word, if not provided uses auto-selection
   */
  override execute(word: string | string[] | null = null): Mind | null {
    // Prime only supports single-word or auto-select. If array passed, take first.
    const singleWord = Array.isArray(word) ? (word[0] ?? null) : word

    logInfo('[Prime] Starting prime operation', {
      providedWord: singleWord,
      autoSelect: !singleWord,
      networkSize: this.network.cues.size,
    })
    ...
    if (!singleWord) {
      ...
      if (primeWords.length === 0) {
        logErr('[Prime] No prime words available, network may be empty')
        return null
      }

      logInfo('[Prime] DMN mode - using multiple prime words', {
    ...
    if (!this.network.hasCue(singleWord)) {
      logWarn('[Prime] Provided word not found in network', { word: singleWord })
      return null
    }

    logInfo('[Prime] Executing single-word prime', {
      word: singleWord,
      cueConnections: this.network.cues.get(singleWord)?.connections?.size ?? 0,
    })

    // Call parent Recall.execute()
    const mind = super.execute(singleWord)

    if (mind) {
      logInfo('[Prime] Prime completed successfully', {
        primeWord: singleWord,
        activatedNodes: mind.activatedCues.size,
        connections: mind.connections.length,
      })
    } else {
      logErr('[Prime] Prime failed', { word: singleWord })
    }
   ...
   * 生成的Mind包含多个激活中心。
   */
  executeMultiple(words: string[]): Mind | null {
    logInfo('[Prime] Starting multi-center prime', {
      words,
      count: words.length,
    })
    ...
    if (!mind) {
      logErr('[Prime] Multi-center prime failed')
      return null
    }

    logInfo('[Prime] Multi-center prime completed', {
