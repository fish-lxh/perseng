...
// 配置保存成功消息修复
// logger.info(`Config saved to: ${chalk.gray('~/.perseng/server-config.json')}`
...
import { PersengMCPServer } from '../servers/PersengMCPServer.js'
...
program
  ...
  .description('Perseng MCP Server - Connect AI applications to Perseng')
...
program
  ...
  .action(async (options) => {
    try {
      logger.info(chalk.cyan(`Perseng MCP Server v${packageJson.version}`))
      ...
      // 使用 PersengMCPServer 统一启动
      await PersengMCPServer.launch({
        transport,
...
${chalk.cyan('More Information:')}
  GitHub: ${chalk.underline('https://github.com/Deepractice/Perseng')}
`)
