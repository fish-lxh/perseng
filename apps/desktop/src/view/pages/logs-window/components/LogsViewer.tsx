    ...
    <div className="h-full flex flex-col min-h-[600px]">  
      <div className="space-y-4 flex flex-col flex-1 overflow-hidden">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4 flex-1 min-h-0">
          {/* 日志列表 */}
          <div className="flex flex-col border border-slate-700 rounded-md p-3 bg-slate-900 min-h-0">
            <div className="font-semibold text-sm mb-2 shrink-0 text-slate-100">{t("logs.list.count", { count: filteredLogs.length })}</div>
            <div className="space-y-2 overflow-y-auto flex-1 min-h-0">
              {filteredLogs.length === 0 && <div className="text-center text-slate-400 py-8">{t("logs.list.empty")}</div>}
              {filteredLogs.map(log => (
                <div
                  key={log.name}
                  className={`p-3 border rounded cursor-pointer transition-colors text-slate-100 ${
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2">
                        {log.type === "error" ? <AlertCircle className="h-4 w-4 text-red-400 shrink-0" /> : <FileText className="h-4 w-4 text-blue-400 shrink-0" />}
                    ...
                    <Button
                      ...
                      className="shrink-0 text-slate-300 hover:text-red-400 hover:bg-slate-800"
                    >
          ...
          {/* 日志内容 */}
          <div className="flex flex-col border border-slate-700 rounded-md p-3 bg-slate-900 min-h-0 overflow-hidden">
            {isLoading && <div className="text-center py-8 text-slate-300">{t("logs.viewer.loading")}</div>}
            {!isLoading && !selectedLog && <div className="text-center text-slate-400 py-8">{t("logs.viewer.selectPrompt")}</div>}
            {!isLoading && selectedLog && (
              <>
                <div className="font-semibold mb-2 shrink-0 text-slate-100">{selectedLog.name}</div>
                <div className="flex-1 overflow-y-auto">
                  <pre className="text-xs font-mono whitespace-pre-wrap wrap-break-word text-emerald-300">{logContent || t("logs.viewer.empty")}</pre>
                </div>
              </>
            )}
          </div>
